function addTeeTime() {
    ...
    for (let i = 0; i < teeSheet.rows.length; i++) {
        ...
        console.log("Row content:", row.cells[2].textContent);

        // Check if the time in the third cell matches the input time
        if (row.cells[2].textContent.trim() === time.trim()) {
            console.log("Time match found:", time);
            
            // Fill the row with the provided information
            for (let j = 3; j <= 7; j++) { // Adjusted indices to match columns
                console.log("in the loop");
                if (row.cells[j].textContent.length <= 1 && playerCount !== 0) {
                    console.log("found this text content");
                    row.cells[j].textContent = booker + '/' + (j - 2);
                    playerCount--;
            ...
            if (playerCount > 0) {
                alert("Invalid entry. Not enough available slots for players.");
                return;
            }
